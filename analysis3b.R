...
#   
#   simulation_dataset <-lapply(seq_along(lags), function(i){
#     lag <- lags[i]
#     theta0s <- thetas - lag
#     
#     simulation_data <- lapply(seq_along(thetas), function(j){
#       list(replicate(n_reps, assortativity.degree(graphon_tensor(funs, n, list(thetas[j], theta0s[j])))))
#     })
# 
#     # simulation_df <- do.call(rbind, lapply(1:length(lags), function(i) {
#     #   lag <- lags[i]
#     #   result_matrix <- do.call(cbind, simulation_data[[i]])  
#     #   
#     #   df_list <- lapply(1:ncol(result_matrix), function(k) {
#     #     data.frame(
...
#     # }))
#     
#     simulation_df <- do.call(rbind, lapply(seq_along(simulation_data), function(j) {
#       result_vector <- simulation_data[[j]]
#       data.frame(
#         x = rep(thetas[j], length(result_vector)), 
#         y = result_vector, 
#         lag = lag
#       )
#     }))
...
#       )
#     
#     summary_stats$color <- cols[i]
#     return(summary_stats)
#   
#   })
#   
#   simulation_all <- do.call(rbind,simulation_dataset)
#   
